...
class CommandService:
    ...
    async def imitate_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        lines = [
            ...
            "",
            "Развлечения:",
            "• /roast @username — добродушная «поджарка» по стилю общения.",
            "• /horoscope @username — персональный гороскоп по паттернам поведения.",
            "• /compatibility @user1 @user2 — шуточный тест совместимости.",
            "• /dialogue @user1 @user2 тема — сгенерировать диалог между пользователями.",
            "• /story @user1 @user2 [тема] — короткий рассказ с участниками.",
